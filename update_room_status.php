...
function updateAllRoomStatus($con) {
    ...
    if ($current_time < '12:00:00') {
        ...
        $today_start = (new DateTime('today'))->setTime(12, 0, 0);
    }
    $today_start_str = $today_start->format('Y-m-d H:i:s');
    $today_end_str = (clone $today_start)->modify('+1 day')->format('Y-m-d H:i:s');
    ...
    while ($room = $rooms_result->fetch_assoc()) {
        ...
        $sql = " SELECT br.booking_room_id, c.check_in, c.check_out 
                 FROM booking_rooms br 
                 JOIN customers c ON br.customer_id = c.customer_id 
                 WHERE br.room_id = ? 
                 AND ( 
                     (c.check_out > ? AND c.check_in < ?) 
                 ) 
        ...
        $stmt->bind_param("iss", $room_id, $today_start_str, $today_end_str);
        if (!$stmt->execute()) {
